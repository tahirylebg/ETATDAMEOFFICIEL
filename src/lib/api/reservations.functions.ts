...
function isRateLimited(identifier: string) {
  const now = Date.now();
  const hits = (rateLimitHits.get(identifier) ?? []).filter(
    (timestamp) => now - timestamp < RATE_LIMIT_WINDOW_MS,
  );

  if (hits.length >= RATE_LIMIT_MAX_REQUESTS) {
    rateLimitHits.set(identifier, hits);
    return true;
  }
...
export const submitReservation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    if (isCrossSiteRequest()) {
      throw new Error("Requête refusée : origine non autorisée.");
    }

    if (isRateLimited(getClientIdentifier())) {
      throw new Error(
        "Trop de demandes de réservation envoyées. Merci de réessayer dans quelques minutes ou d'appeler le restaurant.",
      );
    }
